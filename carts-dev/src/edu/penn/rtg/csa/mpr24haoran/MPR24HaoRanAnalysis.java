...
/**
 * Class MPR24HaoRanAnalysis
 * It's the start point of the MPR24Haoran analysis
 ...
 * NB: The input will specify the number of VCPUs for a component.
 * 	   If the number of VCPUs is not enough for the workload of the component, we will general an invalid interface (which has budget 0)
 * 
 ...
 *
 */
public class MPR24HaoRanAnalysis { 
	...
	public static void run(String inputFilename, String resourceModel, String outputFilename, int whichSchedTest, int sbf_type){
		...
		XMLInterpreter4MPR24HaoRan xmlInterpreter = new XMLInterpreter4MPR24HaoRan(inputFilename); //4 is short for "for"
		...
		XMLWriter4MPR24HaoRan xmlWriter = new XMLWriter4MPR24HaoRan(outputFilename, rootComponent);
	...
	public static int parseSBFType(String str){
		if(str.equalsIgnoreCase("MPR24HaoRan")) return GlobalVariable.MPR_SBF_ARVIND;
		if(str.equalsIgnoreCase("MPR24HaoRan_Meng")) return GlobalVariable.MPR_SBF_MENG;
		
		System.err.println("In parseSBFType() for MPR model! only uspport MPR24HaoRan and MPR24HaoRan_Meng model! System exit.");
		System.exit(1);
		return 0;
	}
