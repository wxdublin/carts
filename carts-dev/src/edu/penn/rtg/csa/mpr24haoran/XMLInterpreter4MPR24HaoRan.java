...
/**
 * Class XMLInterpreter4MPR24HaoRan
...
public class XMLInterpreter4MPR24HaoRan {
	...
	public XMLInterpreter4MPR24HaoRan(String inputFilename) {
		super();
	...
	private void parseNode(Node node, Component parentComponent){
		if(parentComponent == null){
			...
			String numVCPUs = "0";
			if(node.getAttributes().getNamedItem("numVCPUs") == null){
				System.err.println("Must specify numVCPUs as input for the component under this model");
				System.exit(1);
			} else {
				numVCPUs = node.getAttributes().getNamedItem("numVCPUs").getNodeValue();
			}
			
			this.rootComponent = new Component("system", schedulingPolicy, period, numVCPUs, null,true);
		...
		if(parentComponent != null && node.getNodeName().equals("component")){ //non-root component
			...
			Component currentComponent = new Component(name,schedulingPolicy, period, numVCPUs, parentComponent,false);
